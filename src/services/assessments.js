...
export const AssessmentsService = {
  ...
  /**
   * Delete a single form instance (assessment) via the
   * delete_assessment_instance SECURITY DEFINER RPC. The template, the
   * question snapshot, other clients' instances, and the assignment-ledger
   * row are preserved; the ledger row is unlinked (assessment_id set null)
   * so a deleted recurring cycle is never regenerated by the next sweep.
   */
  async deleteInstance(assessmentId) {
    const { data, error } = await supabase.rpc('delete_assessment_instance', {
      p_assessment_id: assessmentId,
    });
    if (error) throw error;
    if (data && data.success === false) {
      throw new Error(data.reason || 'Failed to delete form');
    }
    return data;
  },
