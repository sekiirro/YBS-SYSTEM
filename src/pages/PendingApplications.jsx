...
export default function PendingApplications() {
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="surface-card p-10 text-center text-muted-foreground text-[14px]">
              ...
              <thead>
                <tr className="border-b border-white/[0.06] bg-gradient-to-r from-[#0d1322] to-transparent">
                  {['Applicant', 'Workspace / Brand', 'Package', 'Phone', 'Submitted', 'Status', 'Actions'].map((h) => (
                    <th
                      key={h}
                      className="text-left px-4 py-3 text-[12px] font-semibold uppercase tracking-wider text-muted-foreground"
                    >
                      {h}
                    </th>
                  ))}
                ...
                {filtered.map((a) => (
                  <tr key={a.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] hover:shadow-[inset_2px_0_0_hsl(var(--primary))] transition-all duration-300">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-[12px] text-muted-foreground">{a.applicant_email}</p>
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-[12px] font-medium">{workspaceName(a.assigned_workspace_id)}</p>
                      {a.assigned_workspace_id && (
                        <p className="text-[12px] text-muted-foreground">via registration link</p>
                      )}
                    </td>
                    <td className="px-4 py-3">
            ...
            {filtered.map((a) => (
              <div key={a.id} className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[14px] font-medium">{a.applicant_name}</p>
                    <p className="text-[12px] text-muted-foreground font-mono">{a.applicant_phone}</p>
                  </div>
                  <Badge className={cn(STATUS_STYLE[a.status] || STATUS_STYLE.pending, 'capitalize')}>
                    {a.status.replace(/_/g, ' ')}
                  </Badge>
              ...
              value={
                <>
                  ...
                  {selected.assigned_workspace_id && (
                    <span className="text-[12px] text-muted-foreground ml-1">(via registration link)</span>
                  )}
                </>
              }
            />
